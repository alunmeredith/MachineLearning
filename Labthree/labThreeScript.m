...
AUCfish = trapz(ROCfish(:,1)/100, ROCfish(:,2)/100);
...
% AUC
AUCrand = trapz(ROCrand(:,1)./100, ROCrand(:,2))./100;
...
% AUC
AUCmean = trapz(ROCmean(:,1)/100, ROCmean(:,2)/100);
...
% 3D plot of bayesian pdf
figure(4);
surf(xRange, yRange, (P1./(P1+P2)));

% calculate predictions
for i = 1:(2*N)
    p1(i) = (mvnpdf(X(i,:)', m1, C1) / (mvnpdf(X(i,:)', m1, C1) + mvnpdf(X(i,:)', m2, C2)));
end 
thmin = min(p1);
thmax = max(p1);
thRange = linspace(thmin, thmax, rocResolution);
...
tPos = zeros(rocResolution,1);
fPos = zeros(rocResolution,1);
acc = zeros(rocResolution,1);
% Calculate sensitivity, recall and accuracy for each threshold value. 
for jThreshold = 1:rocResolution
    threshold = thRange(jThreshold);
    tPos(jThreshold) = sum((p1 > threshold) & (y>0)) * (100/N1);
    fPos(jThreshold) = sum((p1 > threshold) & (y<0)) * (100/N2);
